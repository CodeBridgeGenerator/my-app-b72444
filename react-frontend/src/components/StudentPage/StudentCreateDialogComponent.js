...
const StudentCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,college: _entity?.college,
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputText id="college" className="w-full mb-3 p-inputtext-sm" value={_entity?.college} onChange={(e) => setValByKey("college", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["college"]) ? (
              <p className="m-0" key="error-college">
                {error["college"]}
              </p>
            ) : null}
          </small>
